      ...
      {notifications?.length > 0 ? (
        <ul>
          {notifications?.map((notification, index) => {
            const { type, orderId, message, timestamp, items, status } = notification;
            ...
            return (
              ...
              >
                <Link to={`/order/${orderId}`} className="flex items-center gap-3 flex-grow duration-1000">
                  <div className="flex items-center gap-3">
                    <div className="h-2 w-2 mb-4 rounded-full bg-green-300"></div>
                    <div className="leading-none">
                      <h3 className="text-gray-800 text-sm">{message}</h3>
                      {items && (
                        <p className="text-xs text-gray-500">{items.join(", ")}</p>
                      )}
                      {status && (
                        <p className="text-xs text-green-600 font-semibold">Status: {status}</p>
                      )}
