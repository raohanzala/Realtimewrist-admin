...
import { formatAmount, truncateText } from '../helpers'
    ...
    <Box>
      <div className='min-h-64 flex flex-col'>
        ...
        {isLoading ? <SpinnerMini variant='secondary' /> : orders?.length > 0 ?
          <div>
            {
              orders.slice(0, 3).map((order, index) => (<div className={`py-3 flex justify-between ${index !== 2 && 'border-b'} gap-2`} key={order._id}>
                <div className="space-y-1">
                  ...
                  <div className="flex gap-3">
                    <div className="text-sm">{truncateText(order?.items[0]?.name, 10)}</div>
