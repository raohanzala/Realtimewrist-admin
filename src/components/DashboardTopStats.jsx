...
const DashboardTopStats = () => {
  ...
  const statsData = [
    { bgColor: 'bg-blue-100', title: 'Total Orders', isLoadingKey: isUsersLoading, valueKey: 'orders', valueColor: 'text-blue-600', value: totalOrders },
  ...
  return (
    <>
      {statsData.map(({ bgColor, icon, title, isLoadingKey, value, valueColor }, index) => (
        ...
        <div key={index} className={`rounded p-5 py-10 flex flex-col justify-center items-center text-dark-2 ${bgColor}`}>
